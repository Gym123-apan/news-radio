...
function stc(text,ml){ml=ml||200;var r=[];var ss=text.split(/(?<=[。！？；])/);var c='';for(var i=0;i<ss.length;i++){if((c+ss[i]).length>ml&&c.length>0){r.push(c);c=ss[i];}else{c+=ss[i];}}if(c.length>0)r.push(c);return r;}
...
function sc(){if(cc>=cks.length){oe();return;}hl(cc);ui('playing');utt=new SpeechSynthesisUtterance(cks[cc]);utt.lang='zh-CN';utt.rate=parseFloat(document.getElementById('rate').value);utt.pitch=1;utt.volume=1;var v=getCV();if(v)utt.voice=v;utt.onend=function(){cc++;sc();};utt.onerror=function(e){if(e.error==='canceled'||e.error==='interrupted')return;cc++;sc();};sv.speak(utt);}
...
function startSpeech(){if(!sv){document.getElementById('chromeHint').style.display='block';document.getElementById('sts').textContent='此浏览器不支持语音播报，请使用Chrome打开';return;}if(ip&&!ip2)return;if(ip2){sv.resume();ip2=false;ui('playing');return;}sv.cancel();cc=0;var ft=bnt();cks=stc(ft);initEng(function(){ip=true;ip2=false;ui('playing');sc();startKA();});}

function togglePause(){if(!ip)return;if(ip2){sv.resume();ip2=false;ui('playing');}else{sv.pause();ip2=true;ui('paused');}}

function stopSpeech(){sv.cancel();ip=false;ip2=false;cc=0;cks=[];ui('stopped');var its=document.querySelectorAll('.ni');for(var i=0;i<its.length;i++)its[i].classList.remove('ac');document.getElementById('pf').style.width='0%';if(ka){clearInterval(ka);ka=null;}}

function oe(){ip=false;ip2=false;cc=0;cks=[];ui('finished');var its=document.querySelectorAll('.ni');for(var i=0;i<its.length;i++)its[i].classList.remove('ac');document.getElementById('pf').style.width='100%';if(ka){clearInterval(ka);ka=null;}}
...
var ka=null;
function startKA(){if(ka)clearInterval(ka);ka=setInterval(function(){if(sv&&sv.speaking&&!sv.paused){sv.pause();sv.resume();}},8000);}
